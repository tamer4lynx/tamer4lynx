...
import { resolveAndroidPaths, resolveDevToolBootstrap, type DeepLinkConfig, type HostConfig } from '../common/hostConfig';
...
const autolink = (opts?: { includeDevClient?: boolean; release?: boolean }) => {
     ...
     * @param appendIfMissing When false, skip the update if markers are missing (avoids corrupting AndroidManifest.xml).
     */
    function updateGeneratedSection(
        ...
        if (hasBlock) {
            ...
            console.warn(`⚠️ Could not find autolink markers in ${path.basename(filePath)}; skipping update.`);
            return;
        }
    ...
    function ensureProjectActivityDeepLinkMarkers(manifestPath: string): void {
        ...
        if (m.match(selfClosingRe)) {
            m = m.replace(selfClosingRe, (full) => {
                ...
                const openTag = ensureProjectActivityExported(openingLine);
                const inner = `\n        <!-- GENERATED DEEP LINKS START -->\n        <!-- GENERATED DEEP LINKS END -->\n        `;
                return `${openTag}${inner}        </activity>`;
            });
