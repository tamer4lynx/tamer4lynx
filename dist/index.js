...
async function downloadGradle(gradleVersion) {
  ...
  const response = await import_node_fetch.default(gradleBaseUrl);
...
var create = () => {
  ...
  let androidSdk;
  try {
    const configPath = path2.join(process.cwd(), "tamer.config.json");
    if (!fs2.existsSync(configPath)) {
      throw new Error("tamer.config.json not found in the project root.");
    }
    const configRaw = fs2.readFileSync(configPath, "utf8");
  ...
  const rootDir = path2.join(process.cwd(), osName);
  const appDir = path2.join(rootDir, "app");
  const mainDir = path2.join(appDir, "src", "main");
  const javaDir = path2.join(mainDir, "kotlin", kotlinDir);
  const kotlinGeneratedDir = path2.join(javaDir, "generated");
  const assetsDir = path2.join(mainDir, "assets");
  const themesDir = path2.join(mainDir, "res", "values");
  const gradleDir = path2.join(rootDir, "gradle");
  function writeFile(filePath, content, options) {
    fs2.mkdirSync(path2.dirname(filePath), { recursive: true });
    fs2.writeFileSync(filePath, content.trimStart(), options?.encoding ?? "utf8");
  }
  if (fs2.existsSync(rootDir)) {
    console.log(`\uD83E\uDDF9 Removing existing directory: ${rootDir}`);
    fs2.rmSync(rootDir, { recursive: true, force: true });
  }
  console.log(`\uD83D\uDE80 Creating a new Tamer4Lynx project in: ${rootDir}`);
  writeFile(path2.join(gradleDir, "libs.versions.toml"), `
[versions]
agp = "8.9.1"
...
`);
  writeFile(path2.join(rootDir, "settings.gradle.kts"), `
pluginManagement {
    repositories {
        google {
            content {
...
`);
  writeFile(path2.join(rootDir, "build.gradle.kts"), `
...
`);
  writeFile(path2.join(rootDir, "gradle.properties"), `
org.gradle.jvmargs=-Xmx2048m
android.useAndroidX=true
kotlin.code.style=official
android.enableJetifier=true
`);
  writeFile(path2.join(appDir, "build.gradle.kts"), `
plugins {
...
`);
  writeFile(path2.join(themesDir, "themes.xml"), `
<resources>
...
`);
  writeFile(path2.join(mainDir, "AndroidManifest.xml"), `
...
`);
  writeFile(path2.join(kotlinGeneratedDir, "GeneratedLynxExtensions.kt"), `
...
`);
  writeFile(path2.join(javaDir, "Application.kt"), `
package ${packageName}
...
`);
  writeFile(path2.join(javaDir, "TemplateProvider.kt"), `
package ${packageName}
...
`);
  writeFile(path2.join(javaDir, "MainActivity.kt"), `
package ${packageName}
...
`);
  ...
  fs2.writeFileSync(path2.join(assetsDir, ".gitkeep"), "");
  ...
  async function finalizeProjectSetup() {
    if (androidSdk) {
      try {
        ...
        writeFile(path2.join(rootDir, "local.properties"), sdkDirContent);
        console.log("\uD83D\uDCE6 Created local.properties from tamer.config.json.");
      } catch (err) {
        console.error(`❌ Failed to create local.properties: ${err.message}`);
      }
    } else {
      const localPropsPath = path2.join(process.cwd(), "local.properties");
      if (fs2.existsSync(localPropsPath)) {
        try {
          fs2.copyFileSync(localPropsPath, path2.join(rootDir, "local.properties"));
...
var autolink = () => {
  ...
  let packageName;
  try {
    const configPath = path3.join(process.cwd(), "tamer.config.json");
    if (!fs3.existsSync(configPath)) {
      throw new Error("tamer.config.json not found in the project root.");
    }
    const configRaw = fs3.readFileSync(configPath, "utf8");
    const config = JSON.parse(configRaw);
    packageName = config.android?.packageName;
    if (!packageName) {
  ...
  const nodeModulesPath = path3.join(projectRoot, "node_modules");
  const appAndroidPath = path3.join(projectRoot, os3);
  function updateGeneratedSection(filePath, newContent, startMarker, endMarker) {
    ...
    let fileContent = fs3.readFileSync(filePath, "utf8");
...
${endMarker}`;
    if (regex.test(fileContent)) {
      ...
      console.warn(`⚠️ Could not find autolink markers in ${path3.basename(filePath)}. Appending to the end of the file.`);
      fileContent += `
${replacementBlock}
`;
    }
    fs3.writeFileSync(filePath, fileContent);
    console.log(`✅ Updated autolinked section in ${path3.basename(filePath)}`);
  }
  function findTamerPackages() {
    const packages = [];
    ...
    for (const dirName of packageDirs) {
      ...
      const checkPackage = (name, packagePath) => {
        const tamerConfigPath = path3.join(packagePath, "tamer.json");
        if (fs3.existsSync(tamerConfigPath)) {
          try {
            const configRaw = fs3.readFileSync(tamerConfigPath, "utf8");
            const config = JSON.parse(configRaw);
      ...
      if (dirName.startsWith("@")) {
        try {
          const scopedDirs = fs3.readdirSync(fullPath);
          for (const scopedDirName of scopedDirs) {
            const scopedPackagePath = path3.join(fullPath, scopedDirName);
  ...
  function updateSettingsGradle(packages) {
    const settingsFilePath = path3.join(appAndroidPath, "settings.gradle.kts");
...
// Manual edits will be overwritten.`;
    ...
    if (androidPackages.length > 0) {
      androidPackages.forEach((pkg) => {
        ...
        const projectPath = path3.join(pkg.packagePath, sourceDir).replace(/\\/g, "/");
        const relativePath = path3.relative(appAndroidPath, projectPath).replace(/\\/g, "/");
        scriptContent += `
...
println("No native modules found by Tamer4Lynx autolinker.")`;
  ...
  function updateAppBuildGradle(packages) {
    const appBuildGradlePath = path3.join(appAndroidPath, "app", "build.gradle.kts");
    const androidPackages = packages.filter((p) => p.config.android);
...
${implementationLines || "    // No native dependencies found to link."}`;
  ...
  function generateKotlinExtensionsFile(packages, projectPackage) {
    ...
    const generatedDir = path3.join(appAndroidPath, "app", "src", "main", "kotlin", packagePath, "generated");
    const kotlinExtensionsPath = path3.join(generatedDir, "GeneratedLynxExtensions.kt");
    const modulePackages = packages.filter((p) => p.config.android?.moduleClassName);
    const imports = modulePackages.map((p) => `import ${p.config.android.moduleClassName}`).join(`
`);
    const registrations = modulePackages.map((p) => {
...
`;
    ...
    fs3.writeFileSync(kotlinExtensionsPath, kotlinContent.trimStart());
...
var lynxProject = undefined;
try {
  const configPath = path4.join(process.cwd(), "tamer.config.json");
  if (!fs4.existsSync(configPath)) {
    throw new Error("tamer.config.json not found in the project root.");
  }
  const configRaw = fs4.readFileSync(configPath, "utf8");
  const config = JSON.parse(configRaw);
  if (!config.android?.lynxProject) {
    lynxProject = process.cwd();
  } else
    lynxProject = path4.join(process.cwd(), config.android?.lynxProject);
} catch (error) {
  console.error(`❌ Error loading configuration: ${error.message}`);
  process.exit(1);
}
function bundleAndDeploy() {
  ...
  const sourceBundlePath = path4.join(lynxRoot, "dist", "main.lynx.bundle");
  const destinationDir = path4.join(process.cwd(), "android", "app", "src", "main", "assets");
  const destinationBundlePath = path4.join(destinationDir, "main.lynx.bundle");
  try {
    console.log("\uD83D\uDCE6 Starting the build process...");
    execSync2("npm run build", { stdio: "inherit", cwd: lynxRoot });
  ...
  try {
    if (!fs4.existsSync(sourceBundlePath)) {
    ...
    fs4.copyFileSync(sourceBundlePath, destinationBundlePath);
...
function isCocoaPodsInstalled() {
  try {
    execSync3("command -v pod >/dev/null 2>&1");
    return true;
  } catch (error) {
    return false;
  }
...
`);
  ...
  try {
    ...
    const podfilePath = path5.join(rootDir, "Podfile");
    if (!fs5.existsSync(podfilePath)) {
      throw new Error(`Podfile not found at ${podfilePath}`);
    }
    console.log(`\uD83D\uDE80 Executing pod install in: ${rootDir}`);
    execSync3(`pod install`, {
      cwd: rootDir,
      stdio: "inherit"
    });
...
var create2 = () => {
  ...
  let bundleId;
  try {
    const configPath = path6.join(process.cwd(), "tamer.config.json");
    if (!fs6.existsSync(configPath)) {
      throw new Error("tamer.config.json not found in the project root.");
    }
    const configRaw = fs6.readFileSync(configPath, "utf8");
    const config = JSON.parse(configRaw);
  ...
  const rootDir = path6.join(process.cwd(), "ios");
  const projectDir = path6.join(rootDir, appName);
  const xcodeprojDir = path6.join(rootDir, `${appName}.xcodeproj`);
  const bridgingHeader = `${appName}-Bridging-Header.h`;
  function writeFile(filePath, content) {
    fs6.mkdirSync(path6.dirname(filePath), { recursive: true });
    fs6.writeFileSync(filePath, content.trimStart(), "utf8");
  }
  ...
  writeFile(path6.join(rootDir, "Podfile"), `
...
post_install do |installer|
  installer.pods_project.targets.each do |target|
	...
	`);
  writeFile(path6.join(projectDir, "AppDelegate.swift"), `
import UIKit
...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
	...
	`);
  writeFile(path6.join(projectDir, "ViewController.swift"), `
import UIKit
...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
	...
	`);
  writeFile(path6.join(projectDir, "LynxProvider.swift"), `
import Foundation
...
class LynxProvider: NSObject, LynxTemplateProvider {
  func loadTemplate(withUrl url: String!, onComplete callback: LynxTemplateLoadBlock!) {
	...
	`);
  writeFile(path6.join(projectDir, bridgingHeader), `
...
#import <Lynx/LynxModule.h>
	`);
  fs6.mkdirSync(path6.join(projectDir, "Assets.xcassets", "AppIcon.appiconset"), { recursive: true });
  writeFile(path6.join(projectDir, "Assets.xcassets", "AppIcon.appiconset", "Contents.json"), `
{
  "images" : [ { "idiom" : "universal", "platform" : "ios", "size" : "1024x1024" } ],
  "info" : { "author" : "xcode", "version" : 1 }
}
	`);
  fs6.mkdirSync(xcodeprojDir, { recursive: true });
  writeFile(path6.join(xcodeprojDir, "project.pbxproj"), `
// !$*UTF8*$!
{
	archiveVersion = 1;
	classes = {};
...
/* End XCConfigurationList section */
	...
	`);
  fs6.mkdirSync(path6.join(projectDir, "Base.lproj"), { recursive: true });
  writeFile(path6.join(projectDir, "Base.lproj", "Main.storyboard"), `
...
var autolink2 = () => {
  const projectRoot = process.cwd();
  const nodeModulesPath = path7.join(projectRoot, "node_modules");
  const iosProjectPath = path7.join(projectRoot, "ios");
  function updateGeneratedSection(filePath, newContent, startMarker, endMarker) {
...
`;
  ...
  function findTamerPackages() {
    ...
    for (const dirName of packageDirs) {
      ...
      const checkPackage = (name, packagePath) => {
        const tamerConfigPath = path7.join(packagePath, "tamer.json");
        if (fs7.existsSync(tamerConfigPath)) {
          try {
            const configRaw = fs7.readFileSync(tamerConfigPath, "utf8");
            const config = JSON.parse(configRaw);
            if (config.ios) {
              packages.push({ name, config, packagePath });
            }
  ...
  function updatePodfile(packages) {
    const podfilePath = path7.join(iosProjectPath, "Podfile");
    let scriptContent = `  # This section is automatically generated by Tamer4Lynx.
  # Manual edits will be overwritten.`;
    ...
    if (iosPackages.length > 0) {
      iosPackages.forEach((pkg) => {
        ...
        const relativePath = path7.relative(iosProjectPath, path7.join(pkg.packagePath, podspecPath));
        scriptContent += `
...
function bundleAndDeploy2() {
  let lynxProject2;
  let appName;
  try {
    const configPath = path8.join(process.cwd(), "tamer.config.json");
    if (!fs8.existsSync(configPath)) {
      throw new Error("tamer.config.json not found in the project root.");
    }
    const configRaw = fs8.readFileSync(configPath, "utf8");
    const config = JSON.parse(configRaw);
    lynxProject2 = config.ios?.lynxProject ? path8.join(process.cwd(), config.ios.lynxProject) : process.cwd();
    appName = config.ios?.appName;
    if (!appName) {
  ...
  const sourceBundlePath = path8.join(lynxProject2, "dist", "main.lynx.bundle");
  const destinationDir = path8.join(process.cwd(), "ios", appName);
  const destinationBundlePath = path8.join(destinationDir, "main.lynx.bundle");
  try {
    console.log("\uD83D\uDCE6 Starting the build process...");
    execSync4("npm run build", { stdio: "inherit", cwd: lynxProject2 });
    console.log("✅ Build completed successfully.");
  } catch (error) {
    console.error("❌ Build process failed. Please check the errors above.");
    process.exit(1);
  ...
  try {
    if (!fs8.existsSync(sourceBundlePath)) {
      console.error(`❌ Build output not found at: ${sourceBundlePath}`);
      console.error('Please ensure your build process correctly generates "main.lynx.bundle" in the "dist" directory.');
      process.exit(1);
    }
    if (!fs8.existsSync(destinationDir)) {
      console.error(`Destination directory not found at: ${destinationDir}`);
      process.exit(1);
    }
    console.log(`\uD83D\uDE9A Copying bundle to iOS project...`);
    fs8.copyFileSync(sourceBundlePath, destinationBundlePath);
    console.log(`✨ Successfully copied bundle to: ${destinationBundlePath}`);
  } catch (error) {
    console.error("❌ Failed to copy the bundle file.");
    console.error(error.message);
...
android.command("create").description("Create a new Android project").action(() => {
  create_default();
});
android.command("link").description("Link native modules to the Android project").action(() => {
  autolink_default();
});
android.command("bundle").description("Link native modules to the Android project").action(() => {
  bundle_default();
});
...
ios.command("bundle").description("Bundle native modules for the iOS project").action(() => {
  bundle_default2();
});
program.command("link").option("-i, --ios", "Link iOS native modules").option("-a, --android", "Link Android native modules").option("-b, --both", "Link both iOS and Android native modules").option("-s, --silent", "Run in silent mode without outputting messages").description("Link native modules to the project").action(() => {
...
program.command("autolink").description("Auto-link native modules to the project").action(async () => {
  const configPath = path9.join(process.cwd(), "tamer.config.json");
  let config = {};
